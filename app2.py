...
def get_seasons(state):
    ...
    city = fetch_data(seasons_query)
    city = list(city['city'].sort_values(ascending=False))
    return city


def get_teams(state, city):
    ...
    teams = list(teams['market_name'].sort_values(ascending=True))
    return teams
...
def load_match_results(state, city, county):
    results = get_match_results(state, city, county)
